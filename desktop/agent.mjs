...
const INGEST_PROMPT = [
  ...
  '- CONFLICTS: if a capture contradicts an existing page, keep BOTH claims and record a conflict per docs/wiki-schema.md §7 (⚠️ block + .llmwiki/conflicts.json). Do NOT pick a winner — the app resolves it by member vote.',
...
const LINT_PROMPT = [
  ...
  '- CONFLICTS: record contradictions as conflicts per docs/wiki-schema.md §7 (⚠️ block + .llmwiki/conflicts.json); do NOT resolve or reword them.',
