...
const INGEST_PROMPT = [
  ...
  '- CONFLICTS: if a capture CONTRADICTS an existing page, keep BOTH claims. Do NOT pick a winner (the app resolves it by member vote). Record it in BOTH places, in EXACTLY these formats:',
  '  (1) in the page, add/replace this fenced block:',
  '      <!-- llmwiki:conflict -->',
  '      ## ⚠️ 상충 (투표 중)',
  '      - (A) <claim A> — <author or 기존 문서>',
  '      - (B) <claim B> — <author>',
  '      > 앱에서 투표로 정리됩니다.',
  '      <!-- /llmwiki:conflict -->',
  '  (2) append to the JSON ARRAY in .llmwiki/conflicts.json (create the file as [] if missing) an entry EXACTLY shaped:',
  '      {"id":"<unique>","page":"wiki/<file>.md","title":"<short topic>","claims":[{"text":"<claim A>","author":"기존 문서"},{"text":"<claim B>","author":"<capture author>","capturedAt":"YYYY-MM-DD","sourcePath":"inbox/<file>.md"}]}',
  '      ("page" MUST be the full path like "wiki/Foo.md"; "claims" MUST be an array named claims. Do NOT write openedAt/deadline/votes — the app fills timestamps itself; hand-written ones caused timezone bugs.)',
...
const LINT_PROMPT = [
  ...
  '- CONFLICTS: record contradictions with the exact fenced <!-- llmwiki:conflict --> page block AND a .llmwiki/conflicts.json array entry {"id","page":"wiki/<file>.md","title","claims":[{"text","author"},...]} (no openedAt/deadline/votes — the app fills timestamps) — do NOT resolve or reword them.',
