...
3. **At the deadline voting locks**, and the result is NOT applied by itself — press **"✅ Apply the vote result"** on the closed conflict card (**needs an LLM key**; disabled on keyless devices), or it resolves on a key-holding member's **next ingest/reindex**. Majority wins; **no votes / tie → the LLM decides**. On resolution the **page simply states the accepted fact** (no residual note); the vote outcome and the rejected claim are preserved in `wiki/log.md` and git history (non-destructive). Default window is **7 days** (`"conflictVotingDays": N` in the wiki's root `.llmwiki.json`).
